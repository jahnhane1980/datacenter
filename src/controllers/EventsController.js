...
export class EventsController extends BaseController {
    ...
    constructor(tickerRepo, eventRepo, finnhubService, httpClient, pacingManager) {
        super('EventsController', pacingManager);
    ...
    async runDailySync() {
        await this.executeJob('WEEKLY EVENT CALENDAR SYNC', async () => {
    ...
    async runBackfill() {
        await this.executeJob('ALPHA VANTAGE EARNINGS BACKFILL', async () => {
            ...
            await this.processItemsSafely(allTickers, (t) => t.name, async (ticker) => {
                index++;
                console.log(`[${index}/${allTickers.length}] Frage Historie ab für: ${ticker.name}...`);
