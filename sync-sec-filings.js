...
function extractLlmContext(text, keyword, windowSize = 1500) {
    const results = [];
...
/**
 * Sendet aggregierte Textblöcke blitzschnell an die Groq-LPU-Cloud
 */
async function analyzeSnippetWithGroq(snippet, metricName, ticker, archetype) {
    let baseInstruction = '';
...
async function runMasterSync() {
    ...
    const trackedCompanies = await secRepository.getTrackedCompanies();
    ...
    for (const company of trackedCompanies) {
        ...
        // =========================================================================
        // PIPELINE PART 1: Die harten Zahlen aus Yahoo Finance ziehen
        // =========================================================================
        try {
            ...
            if (fundamentalsData && fundamentalsData.length > 0) {
                ...
                for (const quarter of recentQuarters) {
                    ...
                    const exists = await secRepository.fmpFundamentalExists(company.ticker, fiscalYear, period);
                    ...
                    if (!exists) {
                        ...
                        await secRepository.saveFmpFundamentals({
        ...
        // =========================================================================
        // PIPELINE PART 2: Der Text-Sniper mit Groq AI-Radar
        // =========================================================================
        try {
            const companyKeywords = await secRepository.getCompanyKeywords(company.ticker);
            ...
            const recentFilings = await secService.fetchLatestFilings(company.cik, company.is_foreign_issuer, fetchLimit);

            for (const filing of recentFilings) {
                const exists = await secRepository.filingExists(filing.accessionNumber);
                if (exists) {
                ...
                const rawContent = await secService.fetchFilingContent(
                    company.cik,
                ...
                const newFilingId = await secRepository.saveRawFiling(
                    company.ticker,
                    filing.formType,
                    filing.filingDate,
                    filing.accessionNumber,
                ...
                for (const [metricName, keywords] of Object.entries(companyKeywords)) {
                    ...
                    for (const keyword of keywords) {
                        const snippets = extractLlmContext(cleanContent, keyword);
                    ...
                    if (accumulatedSnippets.length > 0) {
                        ...
                        while (retryCount <= maxRetries && !success) {
                            try {
                                ...
                                aiResult = await analyzeSnippetWithGroq(combinedContext, metricName, company.ticker, company.archetype);

                                if (aiResult) {
                                    aiSignalsToSave.push({
                                        filing_id: newFilingId,
                                        ticker: company.ticker,
                                        filing_date: filing.filingDate, // WICHTIG: Das neue Feld für die Datenbank!
                ...
                if (aiSignalsToSave.length > 0) {
                    await secRepository.saveAiSignals(aiSignalsToSave);
